...
const milestones = [
  {
    ...
    emoji: "🏫",
    description: "Started with just 2 classrooms and a big dream.",
  ...
  },
  {
    ...
    emoji: "🏆",
    description: "Student scored in the top 1% nationally.",
  ...
  },
  {
    ...
    emoji: "🔬",
    description: "Modern labs and practical facilities built.",
  ...
  },
  {
    ...
    emoji: "💻",
    description: "Launched e-learning with tablets and smartboards.",
...
const testimonials = [
    {
      quote: "The teachers here really care. I wouldn’t be where I am today without their support.",
      name: "Joyce M., Class of 2010",
      emoji: "💬",
    },
    {
      quote: "Our science fair project made it to nationals! This school sparks curiosity.",
    ...
    {
      quote: "Learning here was fun. I still remember the drama club performances!",
...
const OurHistory: React.FC = () => {
  ...
  return (
    <div className="bg-gray-100 py-16 poppins-regular">
      <h2 className="text-2xl md:text-4xl font-bold text-center mb-12 text-green-800 ">
      🧭 Our School Journey
      </h2>

        {/*Animated timeline  */}
      <div className="max-w-4xl mx-auto relative border-l-4 border-green-500 pl-6 ml-3 md:ml-0">
        {milestones.map((milestone, index) => (
          ...
          >
            <div className="absolute -left-4 top-1.5 w-9 h-9 text-center text-2xl bg-gray-400 rounded-full shadow-md">{milestone.emoji}</div>
            <h3 className="md:text-xl text-lg font-bold text-green-700 pl-9">{milestone.year} - {milestone.title}</h3>
            <p className="text-gray-700 mt-2 pl-6 text-[15px]">{milestone.description}</p>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
    {facts.map((fact, index) => (
      <Tilt key={index} tiltMaxAngleX={10} tiltMaxAngleY={10} glareEnable={true} glareColor="#e0f7fa">
        <motion.div
        ...
        >
          ...
          <h4 className="text-3xl font-bold text-green-800 mb-1">
            <CountUp start={0} end={fact.count} duration={2} separator="," />+
          </h4>
  ...
  >
    ...
    {testimonials.map((testimonial, index) => (
      ...
      >
        ...
        <p className="text-green-800 font-semibold text-right">— {testimonial.name}</p>
      </motion.div>
      </SwiperSlide>
    ))}
</Swiper>
